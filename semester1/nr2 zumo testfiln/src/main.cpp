...
// service og batteribytte
int batterySwapCost = 15;
int batteryServiceCost = 5;
int batteryServiceIncrement = 50;
//eeprom adresse til de forkjellige verdiene
int eeBatteryLevel = 0; // EEPROM 0 - Battery level
int eeBatteryHealth = 1; // EEPROM 1 - Battery health
int eeFundAmount = 2; // EEPROM 2 - Available funds
int eeChargeCyclesCount = 3; // EEPROM 3 - Number of charge cycles
int eeLowBatteryOccurrences = 4; // EEPROM 4 - Number of occurrences of battery level dropping below 5%
int maxValue = 100;
//battery health levels
const uint8_t BATTERY_HEALTH_LOW = 20;
const uint8_t BATTERY_HEALTH_CRITICAL = 10;
...
void nrChargecycles();
void emergancyCheck();
void batteryLowWarning(int batteryHealth);
void batteryChargeDrain(bool emergancy, bool allowReverseCharge);

// Bank functions
void deposit(int amount);
void withdraw(int amount);

// Functions related to battery health
void ProductionFualt();
void batteryServiceMenu();
void batteryService();
void displayMessage(const char* message);
bool isBatteryHealthCritical(uint8_t currentBatteryHealth);
bool isBatteryHealthLow(uint8_t currentBatteryHealth);
void batteryCheck();
void batterySwap();
void wrongActionPrint();
...
void calibrate() { // kalibrere sensorene
    oled.clear();
    oled.gotoXY(0, 0);
...
void updateSensors() { // leser posisjonen til linja
    position = lineSensors.readLine(lineSensorArray);
}

void followLineWithPID() { // Følg linje med PID-regulering
    int16_t error = position - 2000;
    int16_t speedDifference = error / 4 + 6 * (error -
                                               lastError); // Proporsjonal betingelse: error / 4 - Dette er en enkel proporsjonal komponent hvor feilen er delt på 4. Dette betyr at hastighetsforskjellen er proporsjonal med feilen, men skalert ned med 4.
    ...
    leftSpeed = constrain(leftSpeed, 0,
                          normalSpeed);           // Constraining left and right speed to not be lower than 0 or higher than normalSpeed
    rightSpeed = constrain(rightSpeed, 0, normalSpeed);
    motors.setSpeeds(leftSpeed, rightSpeed);                     // Setting the speed for the motors
}

void followline() { // følger linje med p regulering
...
void drive(int motorSpeedParameter) { // kjører begge motor i arg1 hastighet
...
void printValues() {
    ...
    {
        displayMode++;
        if (displayMode == 3) { // reset ved siste displaymodus
            displayMode = 0;
        }
    }
    switch (displayMode) {
        ...
        case 1:
            if (millis() - lastTimePrint > 125) { // tidsdelay så skjermen ikke klikker
                oled.clear();
                oled.setLayout21x8();
                oled.gotoXY(0, 0);
...
void speedometer() {
    ...
    if (millis() - timeLastMinuteUpdateEnd >
        15) // mer nøyaktige verdier dersom den oppdateres med litt mer mellomrom. er orignalt 4ms perioder
    {
        if (timeMotorsOnThisPeriod < 8000) {
            ...
            if (speed > 0) // tiden motoren har kjørt denne perioden
            {
                ...
                if (speed > maxSpeedMinute) { // max hastighetoppdatering
                    maxSpeedMinute = speed;
                }

                if (speed > 45.5) { // tid bilen har kjørt over 70% hastighet denne perioden
        ...
        } else { // først lagre verdiene i displayverdiene som brukes i skjermen så reset de
...
void batteryChargeDrain(bool emergancy,
                        bool allowReverseCharge) // lader opp batteriet når den rygger. NB!: om vi skal utlade tregere må vi sette på et tidsdelay da opp/utladningsverdien nå er 0.01 som er minste verdi mulig
{
    ...
    if (battery_level > 100) { // passer på at batteri nivået ikke blir mer enn 100%.
        battery_level = 100;
...
void driveModeButton() { // knapp for kjøremodus
...
void driveModeBased() { // case med kjør frem tilbake og stå stille ved trykk på knapp B
    switch (driveMode) {
        case 0:
            drive(250);
            break;
...
void batteryService(){
    if(BATTERY_HEALTH_CRITICAL < EEPROM.read(eeBatteryHealth <= BATTERY_HEALTH_LOW)) {
        unsigned int currentBatteryHealth = EEPROM.read(1);
        currentBatteryHealth += batteryServiceIncrement;
        EEPROM.write(eeBatteryHealth, currentBatteryHealth);
        int currentFuncds = EEPROM.read(eeFundAmount);
        currentFuncds -= batteryServiceCost;
        EEPROM.write(eeFundAmount, currentFuncds);
    } else{
        wrongActionPrint();
    }
}
...
void displayMessage(const char* message) {
    oled.clear();
    oled.setLayout21x8(); // Set an appropriate layout
    oled.gotoXY(0, 0); // Set the cursor position
    oled.print((message));
}

bool isBatteryHealthCritical(uint8_t currentBatteryHealth) {
    return currentBatteryHealth <= BATTERY_HEALTH_CRITICAL;
}

bool isBatteryHealthLow(uint8_t currentBatteryHealth) {
    return currentBatteryHealth <= BATTERY_HEALTH_LOW;
}

void batteryCheck() {
    uint8_t currentBatteryHealth = EEPROM.read(1);

    if (isBatteryHealthCritical(currentBatteryHealth)) {
        displayMessage("Battery needs replacement!");
    } else if (isBatteryHealthLow(currentBatteryHealth)) {
        displayMessage("Battery needs service!");
    }
}

void batterySwap(){
    if (EEPROM.read(eeBatteryHealth) <= BATTERY_HEALTH_CRITICAL) {

        EEPROM.write(eeBatteryLevel, maxValue); // Setter batteriet til fullt nivå
        EEPROM.write(eeBatteryHealth, maxValue); // Setter helsen til fullt nivå
        int currentMoney = EEPROM.read(eeFundAmount);
        currentMoney = currentMoney - batterySwapCost;
        EEPROM.write(eeFundAmount, currentMoney);
    } else{
        wrongActionPrint();
    }
}
